"""Deterministic teaching tools, ported verbatim from
`examples/create_agent.ipynb`.

These are classroom stand-ins: no real I/O, no database, no arbitrary code
execution. Each returns curated, deterministic output so students can watch an
agent decide to call a tool and use its result, without any production
integration. The docstrings are intentionally preserved — they are both the
lesson and the tool descriptions the model sees.
"""
...
# Tools in the same order as the notebook, ready to hand to create_agent.
TOOLS = [
    run_sql,
    validate_answer,
    search_docs,
